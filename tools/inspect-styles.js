...
(async () => {
  ...
  // 强制 light 主题便于看筛选区
  await send('Runtime.evaluate', { expression: `localStorage.removeItem('theme');document.documentElement.setAttribute('data-theme','light')`, returnByValue: true });
  await sleep(400);
  await send('Runtime.evaluate', { expression: `document.getElementById('btnToggleFilter')?.click()`, returnByValue: true });
  await sleep(500);

  const sels = ['.brand__mark', '.filter-chip:not(.is-on)', '.filter-chip.is-on', '.filter-panel__label'];
  for (const s of sels) {
    const r = await send('Runtime.evaluate', { expression: inspect(s), returnByValue: true });
