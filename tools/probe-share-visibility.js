...
async function main(){
  ...
  const tmp = '/tmp/cdp-sv-' + process.pid; fs.rmSync(tmp,{recursive:true,force:true});
  ...
  const probeFn = function(){return new Promise(function(res){var t=setInterval(function(){var b=document.querySelector('#btnShare');if(b){clearInterval(t);var cs=getComputedStyle(b);var th=document.querySelector('.toast-host');var ml=document.querySelector('.modal-layer');res({hidden:b.hidden,display:cs.display,toastZ:th?getComputedStyle(th).zIndex:null,modalZ:ml?getComputedStyle(ml).zIndex:null});}},100);setTimeout(function(){clearInterval(t);res({hidden:null,display:null});},6000);});};
  async function evalShare(){ const r=await pageSend('Runtime.evaluate',{expression:'('+probeFn.toString()+')()',returnByValue:true,awaitPromise:true}); return r.result.value; }
  const out={};
  await navigate('http://127.0.0.1:18382/');
  out['18382_noPort']=await evalShare();
  await navigate('http://127.0.0.1:18282/');
  out['18282_withPort']=await evalShare();
